<?php
use libs\Pay\Yeepay\LYeepayTool;
...
class RequestService
{
    ...
    {
        $this->infConfig = LYeepayTool::getInfConfig();
        if (!$bizName) {

            throw new ZGTException("bizName is null, [" . $bizName . "].");
        }

        if (!isViaArray($this->infConfig)) {

            throw new ZGTException("infConfig is null.");
        }

        if (!array_key_exists($bizName, $this->infConfig)) {
        ...
        $this->bizConfig = $this->infConfig[$bizName];
    ...
    public function sendRequest($queryData)
    {
        ...
        if (empty($_FILES["file"]["name"])) {
            ...
            $this->response = post($requestURL, $postfields, array('file' => 'upload/' . $_FILES["file"]["name"]));
        }
    ...
    public function receviceResponse()
    {
        ...
        if (array_key_exists("code", $responseJsonArray) && "1" != $responseJsonArray["code"]) {
